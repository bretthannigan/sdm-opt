...
% Attempt at designing the loop filter directly through a transform, to
% enforce realizability condition.
...
G(1) = (B{1} - ((1 - gam)/gam)^2*A{1})/2;
for iG=2:(n+1)
    G(iG) = (B{1} - ((1 - gam)/gam)^2*A{1}); %#ok<SAGROW>
end
lmi{3} = F'*(kron(-Phi, P) + kron(Psi, Q))*F - (G'*J + J'*G);
ymConstraint = [ymConstraint, lmi{3}<=0, Q>=0];

% G_pb(1) = -(((1 - gam_pb)/gam_pb)^2*A{1} - B{1})/2;
% for iG_pb=2:(n+1)
%     G_pb(iG_pb) = -(((1 - gam_pb)/gam_pb)^2*A{1} - B{1}); %#ok<SAGROW>
% end
