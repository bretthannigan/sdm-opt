...
ymConstraint = [ymLmi{1}<=0, ymLmi{2}<=0, ymLmi{3}>=0];

ymSolver = 'SDPT3'; %'SeDuMi';
ymOptions = sdpsettings('verbose', true, 'solver', ymSolver, 'sdpt3.inftol', 1e-9, 'sdpt3.gaptol', 1e-9);
ymObjective = gam_gkyp + trace(aa);
